import { ROLE_ORDER, DEPARTMENTS, SOLDIER_ROLE_ID } from "./config/roles.js";

// Все роли (по id), которые показаны в отделах — их исключаем из Soldiers
const DEPT_ROLE_IDS = new Set(DEPARTMENTS.flatMap(d => d.roles));
...
async function loadDiscordMembers() {
  try {
    ...
    allMembers = members
      .filter(m =>
        m.roles?.some(r => r.id === SOLDIER_ROLE_ID || ROLE_ORDER.includes(r.name)) &&
        !m.roles.some(r => DEPT_ROLE_IDS.has(r.id))
      )
